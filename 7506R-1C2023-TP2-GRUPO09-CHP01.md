...
y_train_transformed = y_train_map(lambda x: 0 if x == 'positivo' else 1)
```
...
if not exists('modelos/TP2/metricas_nb.joblib')
    ...
    resultados_nb = cross_validate(modelo_final, x_train.review_es, y_train, cv=kfoldcv,scoring=metrica ,return_estimator=False)
    metricas_nb = resultados_xgb['test_score']
    xgb_tuneado = resultados_xgb['estimator'][np.where(metricas_nb==max(metricas_nb))[0][0]]
else: 
...
```python
y_test.map({'positivo': 0, 'negativo': 1})
