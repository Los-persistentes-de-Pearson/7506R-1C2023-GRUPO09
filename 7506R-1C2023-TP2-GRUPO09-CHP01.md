...
```python
if exists('modelos/TP2/modeloRandomForest-sin-optimizar-3.joblib') == False:
    ...
    dump(modeloRandomForest, 'modelos/TP2/modeloRandomForest-sin-optimizar-3.joblib')

else:
    modeloRandomForest = load('modelos/TP2/modeloRandomForest-sin-optimizar-3.joblib')
```
...
```python
if exists("modelos/TP2/modeloRandomForest-sin-optimizar-2.joblib") == False:
    #Nos guardamos este modelo para poder cargarlo en todas las corridas posteriores
    dump(modeloRandomForest, 'modelos/TP2/modeloRandomForest-sin-optimizar-2.joblib')
else:
    modeloRandomForest = load('modelos/TP2/modeloRandomForest-sin-optimizar-2.joblib')
```

Hacemos la submission

```python
if not exists('submissions/TP2/randomForest-sin-opti-3.csv'):
    ...
    df_submission.to_csv('submissions/TP2/randomForest-sin-opti-3.csv', index=False)
```
...
if exists('modelos/TP2/GsRandomForest_y_tdif-1.joblib') == False:
    ...
    param_grid = { "gsrf__criterion" : ["gini"], 
                   "gsrf__min_samples_leaf" : [15], #Vamos a hacer muchas combinaciones ya que solo vamos
                   "gsrf__min_samples_split" : [40],#a correr este modelo 1 sola vez; ya que lo vamos a 
                   "gsrf__n_estimators": [40],  #guardar   
...
```python
if not exists ('modelos/TP2/GsRandomForest_y_tdif-1.joblib'):
    dump(mejor_modelo_rf, "modelos/TP2/GsRandomForest_y_tdif-1.joblib")
```
...
Sin embargo obtuvimos una mejora en la complejidad del modelo (pes)a 1.5 mb menos! #TODO
...
```python
if not exists('submissions/TP2/randomForest-y_tdif-1.csv'):
    ...
    df_submission.to_csv('submissions/TP2/randomForest-y_tdif-1.csv', index=False)
```
...
Ajustamos los valores para que los entienda el modelo

```python
# y_test = y_test.map({'positivo': 0, 'negativo': 1})
# y_train.map({'positivo': 0, 'negativo': 1})
# y_test
y_test_filtrado = np.array(list(map(lambda x: 1 if x=="positivo" else 0, y)))
y_train = np.array(list(map(lambda x: 1 if x=="positivo" else 0, y)))
len(y_test_filtrado)
```
...
if not exists('modelos/TP2/xgb_tuneado.joblib'):
    ...
    xgb_tuneado = load('modelos/TP2/xgb_tuneado.joblib')
    metricas_xgb = load('modelos/TP2/metricas_xgb.joblib')
...
Reajustamos el y_test y el y_train

```python
y_test.map({0: 'positivo', 1: 'negativo'})
y_train.map({0: 'positivo', 0: 'negativo'})
```
...
if not exists('submissions/TP2/redesNeuronales2.csv'):
    ...
    df_submission = pd.DataFrame({'id': review_pruebasOrigposmodernoinal['ID'], 'sentimiento': yEnEspanol})
...
def creador_modelo(learning_rate = 0.1, 
    ...
    modeloHiper.add(keras.layers.Embedding(vocab_size + num_oov_buckets, embed_size,
        input_shape=[None]))
