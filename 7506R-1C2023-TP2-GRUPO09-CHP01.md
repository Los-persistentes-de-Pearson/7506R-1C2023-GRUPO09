...
```py
if !exists('modelos/xgb_base.joblib'):
...
```py
if exists('submissions/xgboost_base.csv'):
...
if !exists('modelos/hipers_xgb.joblib'):
...
```py 
if !exists('modelos/xgb_tuneado.joblib'):
...
Realizo validacion cruzada del modelo para observar su comportamiento 
