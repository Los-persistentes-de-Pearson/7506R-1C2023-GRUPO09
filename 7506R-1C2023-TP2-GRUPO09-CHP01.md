...
if not exists('modelos/xgb_base.joblib'):
    ...
    xgb_base = joblib.load("modelos/xgb_base.job")
```
...
if not exists('modelos/hipers_xgb.joblib'):
    ...
    dump(modelo_rcv, 'modelos/hipers_xgb.joblib')
else: 
    modelo_rcv = load('modelos/hipers_xgb.joblib')
```
...
```py 
if not exists('modelos/xgb_tuneado.joblib'):
    ...
    xgb_tuneado = load('modelos/xgb_tuneado.joblib')
```
...
word for word, count in vocabulary2.most_common()[:vocab_size]]
```
...
table2 = tf.lookup.StaticVocabularyTable(vocab_init, num_oov_buckets)
```
...
#     fila = fila[:300]
    tokenDeFrase = []
    for word in fila:
        tokenDeFrase.append(int(table.get(bytes(word, 'utf-8'))))
#         fila[i] = bytes(fila[i], 'utf-8')
    numerico.append(np.array(tokenDeFrase))
    i+=1
```

```python
xTEST = pd.DataFrame({'x': numerico, 'y':y_train})
```

```python
numerico
# numerico = np.asarray(numerico).astype(np.float32)
```
...
# train_x, train_y = encode_words(x_train, y_train)
```
...
```python
tf.convert_to_tensor(xTEST['x'])
```

```python
if exists('modelos/TP2/redNeuronalSentimiento2.joblib') == False:
#     historia_modelo=redNeuronal.fit(train_set, epochs=4)
    historia_modelo=redNeuronal.fit(x = tf.convert_to_tensor(xTEST['x']), 
                                    y = tf.convert_to_tensor(xTEST['y']), epochs=4)
...
#     rs_fit = rs.fit(X = x_train_escalado, y = y_train)
    rs_redNeuronal=rs.fit(X = xTEST['x'], y = xTEST['y'])
    rs_redNeuronal=rs.fit(X = xTEST['x'], y = xTEST['y'])

    dump(rs_fit, 'modelos/TP2/rs_redNeuronal.joblib')
else:
    rs_fit = load('modelos/TP2/rs_redNeuronal.joblib')
```
